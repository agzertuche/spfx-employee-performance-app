...
export default class SPPnPDataProvider implements IDataProvider {
  ...
  private _getEmployeeInformation(): Promise<IEmployeeInformation[]> {
    return sp.web.lists.getByTitle('Employees').items.getAll();
  }

  private _getAchievements(): Promise<IAchievement[]> {
    return sp.web.lists
      .getByTitle('Achievements')
      .items.getAll()
      .then(results => {
  ...
  private _getEarnedAchievements(): Promise<any[]> {
    return sp.web.lists
      .getByTitle('Earned Achievements')
      .items.getAll()
      .then(results => {
        return results.map(x => {
          return {
  ...
  private _getPerformanceSkills(): Promise<IPerformanceSkills[]> {
    return sp.web.lists.getByTitle('Performance Skills').items.getAll();
  }

  public set webPartContext(value: any) {
    this._webPartContext = value;
  }

  public get webPartContext(): any {
